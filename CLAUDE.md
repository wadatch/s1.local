...
`/graphs` の表示条件（期間・並べ方・表示・選んだセンサー）は名前をつけて
保存できる。センサーを選び直すのが毎回同じ手間になるため
（「夏の南棟だけ」「寝室の 7 日」など）。

**前回の続き（`STORAGE_KEY`）とは別の場所に置く（`PRESETS_KEY`）。**
前回の続きは操作のたびに黙って上書きされるものなので、同じ場所に置くと
名前をつけて残したものが消える。

保存先はこのブラウザ。端末ごとに見たいものが違うので、サーバに置く意味がない。

- **同じ名前は上書き**。2 つ残すと、どちらが新しいか分からなくなる
- **消すときは確認する**。押し間違いで消えると、何を保存していたか思い出せない
- 保存したセンサーが今も居るとは限らない（取得を止めた・撤去した）。
  `applySelection()` は**居ないものを黙って飛ばす**
- 条件を当てる前に**今の選択を全部外す**。外さないと、当てる側に無いセンサーが
  残って「保存したときの条件」と画面が食い違う

今の条件が保存済みのどれかと一致していれば、その名前を印として出す
（`matchingPresetName()`）。畳んだときの要約にも名前が出る。
一致の判定でセンサーは**並べ替えてから**比べる。選んだ順は条件の違いではない。
